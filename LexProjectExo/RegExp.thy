...
datatype 'a rexp = Empty                         ("<>")
                 ...
                 | Star "('a rexp)"              
                 | Interleave "('a rexp)" "('a rexp)" (infix "||" 62)
...
fun interleave :: "'a list \<Rightarrow> 'a list \<Rightarrow> 'a list set" where
    "interleave [] S = {S}"
  | "interleave S [] = {S}"
  | "interleave (a#S) (b#T) = (#) a ` interleave (S) (b#T)  \<union> (#) b ` interleave (a#S) (T) "
    
value "interleave [1::int,2] [3,4,5]"

definition Interleave :: "'a list set \<Rightarrow> 'a list set \<Rightarrow> 'a list set"
  where   "Interleave S T = (\<Union>x\<in>S. \<Union>y\<in>S. interleave x y)"
...
(*     |L_Interleave "L (el || er) = interleave" *)
